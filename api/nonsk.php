...
$amount = 1;
$sk_file = file("sknahidhundpayegarandwabhosdiwaleterimakichutuwkaowowudueuwoqowoowsldmxmnfbc be hdnanakakwowieueyyeyeuwiwowoeieuryryjsbzbznn¶`×|÷•°•°•°82(3+47383737×`÷|÷.txt");
$get_sk = end($sk_file); 
$sk= trim($get_sk);
...
$status = '#CVV';
$resmsg = $cur.$amount.' Charged BY @fakehecker';
' <p class="uk-margin-small-top">'.$status.' : '.$resmsg.' : ' . $lista . ' : '.$country.' : <a class="receipt" href="'.$receipturl.'">Get Receipt</a> - </p>';
if(strpos($seller_message, '"seller_message": "Payment complete."' )) {
exit;
}elseif ((strpos($r2,'insufficient_funds')) || (strpos($r3,'insufficient_funds'))){
$status = '#CVV';
$resmsg = 'Insufficient BY @fakehecker';
}elseif (strpos($r3, "incorrect_cvc") || strpos($r2, "incorrect_cvc")) {
    $status = '#CCN';
$resmsg = 'Incorrect cvc BY @fakehecker';
